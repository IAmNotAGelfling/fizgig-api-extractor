...
def extract(
    ...
    format: str = typer.Option(
        "markdown", "--format", "-f", help="Output format: markdown, csv, json, html"
    ),
    ...
    template: Optional[str] = typer.Option(
        None, "--template", "-t", help="Path to custom HTML template (HTML format only)"
    ),
) -> None:
    """
    ...
    try:
        ...
        if config:
            ...
            cli_overrides = {
                "input": input_file
                if input_file != "api.json"
                else None,  # Only override if not default
        ...
        data, detected_format = load_api_file(
            input_file, headers=headers_dict, save_path=save_path
        )
        ...
        console.print(
            f"[green]✓[/green] Detected format: [bold]{detected_format}[/bold]"
        )
        ...
        console.print(
            f"[green]✓[/green] Extracted [bold]{len(endpoints)}[/bold] endpoint(s)"
        )
...
@app.command()
def convert(
    input_file: str = typer.Argument(..., help="Path to OpenAPI spec (JSON/YAML)"),
    ...
    try:
        ...
        if detected_format != "openapi":
            console_err.print(
                f"[red]✗[/red] Input file is not an OpenAPI spec (detected: {detected_format})"
            )
            raise typer.Exit(1)
        ...
        console.print(
            f"[green]✓[/green] Converted [bold]{total_items}[/bold] endpoint(s)"
        )
...
def tree(
    ...
    show_params: bool = typer.Option(
        False, "--params", "-p", help="Show parameter details"
    ),
) -> None:
    """
...
def init(
    config_only: bool = typer.Option(
        False, "--config-only", help="Only create config file, skip template"
    ),
    template_only: bool = typer.Option(
        False, "--template-only", help="Only create template, skip config file"
    ),
    ...
    try:
        ...
        if not template_only:
            ...
            if config_path.exists():
                console_err.print(
                    f"[red]✗[/red] Config file already exists: {config_path}"
                )
                raise typer.Exit(1)
            ...
            example_config = {
                ...
                "exports": [
                    {"format": "markdown", "output": "docs/api-endpoints.md"},
                    {
                        "format": "csv",
            ...
            console.print(
                f"[green]✓[/green] Created config file: [bold]{config_path}[/bold]"
            )
        ...
        if not config_only:
            ...
            if template_path.exists():
                console_err.print(
                    f"[red]✗[/red] Template file already exists: {template_path}"
                )
                raise typer.Exit(1)
            ...
            try:
                template_content = (
                    ...
                    .joinpath("templates/default.html")
            ...
            console.print(
                f"[green]✓[/green] Created template file: [bold]{template_path}[/bold]"
            )

        console.print("\n[bold]Next steps:[/bold]")
        if not template_only:
            console.print(
                "1. Edit .fizgig-config.json with your API spec path and export settings"
            )
        if not config_only:
            console.print("2. Customize templates/default.html if needed")
        console.print(
            "3. Run: fizgig-api-extractor extract --config .fizgig-config.json"
        )
...
def validate_config(
    ...
    try:
        ...
        if validation_result["valid"]:
            ...
            console.print(
                f"  Exports: [bold]{len(config['exports'])}[/bold] configured"
            )
            return
